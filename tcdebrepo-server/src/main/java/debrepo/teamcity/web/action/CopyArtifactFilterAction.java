...
public class CopyArtifactFilterAction extends ArtifactFilterAction implements ControllerAction {

	private final DebRepositoryConfigurationManager myDebRepositoryConfigurationManager;
	private final static String COPY_FILTER_ACTION = "copyArtifactFilter";

	public CopyArtifactFilterAction(@NotNull final DebRepositoryConfigurationManager debRepositoryConfigurationManager,
	...
	public void process(@NotNull final HttpServletRequest request, @NotNull final HttpServletResponse response,
		...
		if (debConfig != null && buildTypeId != null && filter != null) {
			...
			if (!result.isError()) {
				ActionMessages.getOrCreateMessages(request).addMessage("filterUpdateResult", "Artfact Filter copied");
				ajaxResponse.setAttribute("status", "OK");
			} else {
				ActionMessages.getOrCreateMessages(request).addMessage("filterUpdateResult", "Sorry, the Artifact Filter could not be copied.");
