...
AND component = 'main' AND arch = 'amd64' AND PACKAGES_FILE_NAME  = 'Packages'  AND ID NOT IN
 ...
 SELECT id, FILE_CONTENT from O_DEB_METADATA_FILE where ID IN 
 ( SELECT ID FROM O_DEB_METADATA_FILE T1
   WHERE MODIFIED_TIME = 
   ( SELECT max(MODIFIED_TIME) 
     FROM O_DEB_METADATA_FILE 
		WHERE O_DEB_METADATA_FILE.REPOSITORY_ID = 3
      AND O_DEB_METADATA_FILE.DIST = 'wheezy'
		 AND O_DEB_METADATA_FILE.FILE_NAME = 'Release'
...
select * from O_DEB_METADATA_FILE 
